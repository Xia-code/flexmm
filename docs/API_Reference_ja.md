...
本書では現在、ソースコードから抽出した **160** 個のクラス、メソッド、モジュールレベル関数を記載しています。

シグネチャまたは docstring を変更した場合は、ドキュメントとコードの同期を保つため本ファイルを再生成してください。概念的なワークフロー説明とエンドツーエンドの例については、リポジトリの [README](https://github.com/Xia-code/flexmm) を参照してください。
